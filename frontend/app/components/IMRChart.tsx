...
import type { IMRChartPoint } from '../lib/api';
...
export default function IMRChart({ points, x_bar, mr_bar, sigma, k }: IMRChartProps) {
  const svgRef = useRef<SVGSVGElement>(null);
  const [tooltip, setTooltip] = useState<{ x: number; y: number; point: IMRChartPoint } | null>(null);
  ...
  const { xScale, yScale, linePath, uclPath, lclPath, centerY, xTicks, yTicks, visibleIndices } = useMemo(() => {
    if (points.length === 0) {
      return { xScale: () => 0, yScale: () => 0, linePath: '', uclPath: '', lclPath: '', centerY: 0, xTicks: [] as { label: string; x: number }[], yTicks: [] as number[], visibleIndices: new Set<number>() };
    }
    ...
    if (allValues.length === 0) allValues.push(x_bar);
    const minV = Math.min(...allValues, x_bar);
    const maxV = Math.max(...allValues, x_bar);
    ...
    const linePath = points
      .map((pt, i) => {
        ...
        lineStarted = true;
        return `${cmd}${xScale(times[i])},${yScale(pt.x)}`;
      })
      .join(' ');
    ...
    return { xScale, yScale, linePath, uclPath, lclPath, centerY, xTicks, yTicks, visibleIndices };
  }, [points, x_bar, innerW, innerH]);
  ...
  const handleMouseMove = (e: React.MouseEvent<SVGSVGElement>) => {
    ...
    if (allVals.length === 0) allVals.push(x_bar);
    const minV = Math.min(...allVals, x_bar);
    const maxV = Math.max(...allVals, x_bar);
    const pad = Math.max((maxV - minV) * 0.15, 0.5);
    const yMin = minV - pad;
    const yMax = maxV + pad;
    const yRange = yMax - yMin || 1;
    ...
    const xVal = points[closest].x ?? x_bar;
    const py = innerH - ((xVal - yMin) / yRange) * innerH + margin.top;
    ...
    return (
      <div className={styles.wrapper}>
        <p className={styles.empty}>
          Not enough data for an I-MR chart yet. At least 2 runs are required.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className={styles.wrapper}>
      <h3 className={styles.title}>I-MR Chart (Individuals)</h3>
      <div className={styles.stats}>
        <span className={styles.stat}>
          x&#772; = {x_bar.toFixed(3)}
        </span>
        <span className={styles.stat}>
          MR&#772; = {mr_bar.toFixed(3)}
        </span>
        <span className={styles.stat}>
          ...
          >
            ...
            <div className={styles.tooltipValue}>
              <span className={tooltip.point.out_of_control ? styles.tooltipOoc : styles.tooltipNormal}>
                x = {tooltip.point.x != null ? tooltip.point.x.toFixed(3) : '—'}
              </span>
            </div>
            {tooltip.point.mr != null && (
              <div className={styles.tooltipMeta}>MR = {tooltip.point.mr.toFixed(3)}</div>
            )}
            {tooltip.point.lcl != null && tooltip.point.ucl != null && (
              <div className={styles.tooltipMeta}>
                LCL {tooltip.point.lcl.toFixed(3)} &ndash; UCL {tooltip.point.ucl.toFixed(3)}
              </div>
            )}
      ...
      <div className={styles.legend}>
        <span className={styles.legendItem}>
          <span className={styles.legendLine} data-kind="data" /> Observed x
        </span>
        <span className={styles.legendItem}>
          <span className={styles.legendLine} data-kind="center" /> x&#772; (center)
        </span>
        <span className={styles.legendItem}>
