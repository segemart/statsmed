...
export default function LaneyUChart({ points, ubar, sigma_z, k }: LaneyUChartProps) {
  const svgRef = useRef<SVGSVGElement>(null);
  const [tooltip, setTooltip] = useState<{ x: number; y: number; point: LaneyUChartPoint } | null>(null);
  ...
  const { xScale, yScale, linePath, uclPath, lclPath, uclIndPath, lclIndPath, centerY, xTicks, yTicks } = useMemo(() => {
    ...
    if (allValues.length === 0) allValues.push(ubar);
    const minV = Math.min(...allValues, ubar);
    const maxV = Math.max(...allValues, ubar);
    ...
    const linePath = points
      .map((pt, i) => `${i === 0 ? 'M' : 'L'}${xScale(times[i])},${yScale(pt.u)}`)
      .join(' ');
  ...
  const handleMouseMove = (e: React.MouseEvent<SVGSVGElement>) => {
    ...
    if (allVals.length === 0) allVals.push(ubar);
    const minV = Math.min(...allVals, ubar);
    const maxV = Math.max(...allVals, ubar);
    const pad = Math.max((maxV - minV) * 0.15, 0.5);
    const yMin = Math.max(0, minV - pad);
    const yMax = maxV + pad;
    const yRange = yMax - yMin || 1;
    ...
    const py = innerH - ((points[closest].u - yMin) / yRange) * innerH + margin.top;
    ...
    return (
      <div className={styles.wrapper}>
        <p className={styles.empty}>
          Not enough data for a Laney u&prime; chart yet. At least 2 runs are required.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className={styles.wrapper}>
      <h3 className={styles.title}>Laney u&prime; Chart</h3>
          ...
          >
            ...
            <div className={styles.tooltipValue}>
              <span className={tooltip.point.out_of_control ? styles.tooltipOoc : styles.tooltipNormal}>
                u = {tooltip.point.u.toFixed(4)}
              </span>
            </div>
            <div className={styles.tooltipMeta}>count = {tooltip.point.count}, n = {tooltip.point.n}</div>
            {tooltip.point.lcl != null && tooltip.point.ucl != null && (
              <div className={styles.tooltipMeta}>
                LCL {tooltip.point.lcl.toFixed(4)} &ndash; UCL {tooltip.point.ucl.toFixed(4)} (avg n)
              </div>
            )}
            {tooltip.point.lcl_individual != null && tooltip.point.ucl_individual != null && (
              <div className={styles.tooltipMeta}>
                LCL {tooltip.point.lcl_individual.toFixed(4)} &ndash; UCL {tooltip.point.ucl_individual.toFixed(4)} (n={tooltip.point.n})
              </div>
            )}
      ...
      <div className={styles.legend}>
        <span className={styles.legendItem}>
          <span className={styles.legendLine} data-kind="data" /> Observed u
        </span>
        <span className={styles.legendItem}>
          <span className={styles.legendLine} data-kind="center" /> ū (center)
        </span>
        <span className={styles.legendItem}>
